defmodule LiveViewPersonWeb.CoreComponents do
  @moduledoc """
  ...
  import LiveViewPersonWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LiveViewPersonWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LiveViewPersonWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Renders a side peek panel.

  ## Examples

      <.sidepeek id="my-panel" show={@show_panel}>
  ...
  the closing/cancel event, for example:
      ...
      <.sidepeek id="my-panel" show={@show_panel} on_cancel={JS.navigate(~p"/posts")}>
        <:title>Panel Title</:title>
        Panel content goes here
      </.sidepeek>
  """
          ...
          <div class="flex min-h-0 flex-1 flex-col">
            <div class="px-4 py-6 sm:px-6">
              <div class="flex items-start justify-between">
                <h2 id={"#{@id}-title"} class="text-lg font-medium text-zinc-900">
                  <%= render_slot(@title) %>
                </h2>
  ...
  def show_sidepeek(js \\ %JS{}, id) when is_binary(id) do
    js
    |> JS.show(to: "##{id}")
    |> JS.show(
      to: "##{id}-bg",
